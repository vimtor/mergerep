...
export function calculateScore(
  ...
  const totalStars = external.reduce(
    (sum, pr) => sum + pr.repository.stargazerCount,
    0,
  );
...
export function calculateStats(
  ...
  const externalIssues = issues.filter((i) =>
    isRelevant(i.repository.owner.login, i.repository.stargazerCount, username),
  );
